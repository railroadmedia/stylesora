module.exports = (theme) => {
    //Buttons
    return {
        '.btn-primary, .btn-secondary': {
            alignItems: 'center',
            borderStyle: 'solid',
            borderRadius: theme('borderRadius.full'),
            borderWidth: theme('borderWidth.2'),
            color: 'white',
            cursor: 'pointer',
            display: 'inline-flex',
            fontFamily: '"Roboto Condensed", Segoe UI, sans-serif',
            fontSize: '1rem',
            fontWeight: '700',
            justifyContent: 'center',
            lineHeight: '1',
            marginBottom: '0.25rem !important',
            minHeight: '50px',
            padding: '8px 70px',
            position: 'relative',
            textDecoration: 'none',
            textTransform: 'uppercase',
            transitionDuration: '150ms',
            transitionProperty: 'background-color, color, fill, stroke, opacity, box-shadow, transform',
            transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)',
            userSelect: 'none',
            whiteSpace: 'nowrap',
            '&:focus-visible': {
                outline: 'none'
            }
        },
        //Primary
        '.btn-primary': {
            borderColor: 'transparent',
            '&:visited': {
                backgroundColor: 'currentColor',
                color: 'white'
            },
            '&:hover' : { textDecoration: 'none' },
            '&[disabled], &[disabled]:active': {
                backgroundColor: theme('backgroundColor.gray.200')+ '!important',
                color: theme('textColor.gray.400') + '!important',
                pointerEvents:'none',
            },
            //Brand Specific
            //hover
            '&.bg-drumeo:hover, &.bg-drumeo:not([disabled]):focus-visible': {
                backgroundColor: theme('backgroundColor.drumeo.400') + '!important',
            },
            '&.bg-pianote:hover, &.bg-pianote:not([disabled]):focus-visible': {
                backgroundColor: theme('backgroundColor.pianote.300') + '!important',
            },
            '&.bg-guitareo:hover, &.bg-guitareo:not([disabled]):focus-visible': {
                backgroundColor: theme('backgroundColor.guitareo.400') + '!important',
            },
            '&.bg-singeo:hover, &.bg-singeo:not([disabled]):focus-visible': {
                backgroundColor: theme('backgroundColor.singeo.400') + '!important',
            },
            '&.bg-recordeo:hover, &.bg-recordeo:not([disabled]):focus-visible': {
                backgroundColor: theme('backgroundColor.recordeo.400') + '!important',
            },
            //active
            '&.bg-drumeo:not([disabled]):active': {
                backgroundColor: theme('backgroundColor.drumeo.600') + '!important',
            },
            '&.bg-pianote:not([disabled]):active': {
                backgroundColor: theme('backgroundColor.pianote.600') + '!important',
            },
            '&.bg-guitareo:not([disabled]):active': {
                backgroundColor: theme('backgroundColor.guitareo.600') + '!important',
            },
            '&.bg-singeo:not([disabled]):active': {
                backgroundColor: theme('backgroundColor.singeo.600') + '!important',
            },
            '&.bg-recordeo:not([disabled]):active': {
                backgroundColor: theme('backgroundColor.recordeo.600') + '!important',
            }
        },
    
        //Secondary
        '.btn-secondary': {
            borderColor: 'currentColor',
            backgroundColor: 'transparent',
            '&:hover': {
                textDecoration: 'none',
            },
            '&[disabled], &[disabled]:active': {
                pointerEvents:'none',
                color: theme('textColor.gray.400') + '!important',
            },
            //Brand Colors
            '&.text-drumeo:hover, &.text-drumeo:not([disabled]):focus-visible': {
                color: theme('textColor.drumeo.400') + '!important',
            },
            '&.text-pianote:hover, &.text-pianote:not([disabled]):focus-visible': {
                color: theme('textColor.pianote.300') + '!important',
            },
            '&.text-guitareo:hover, &.text-guitareo:not([disabled]):focus-visible': {
                color: theme('textColor.guitareo.400') + '!important',
            },
            '&.text-singeo:hover, &.text-singeo:not([disabled]):focus-visible': {
                color: theme('textColor.singeo.300') + '!important',
            },
            '&.text-recordeo:hover, &.text-recordeo:not([disabled]):focus-visible': {
                color: theme('textColor.recordeo.400') + '!important',
            },
            //Active Colors
            '&.text-drumeo:not([disabled]):active': {
                color: theme('textColor.drumeo.600') + '!important',
            },
            '&.text-pianote:not([disabled]):active': {
                color: theme('textColor.pianote.600') + '!important',
            },
            '&.text-guitareo:not([disabled]):active': {
                color: theme('textColor.guitareo.600') + '!important',
            },
            '&.text-singeo:not([disabled]):active': {
                color: theme('textColor.singeo.600') + '!important',
            },
            '&.text-recordeo:not([disabled]):active': {
                color: theme('textColor.recordeo.600') + '!important',
            }
        },
        
        //Button Variants,
        '.btn-circle': {
            padding: '0 !important',
            height: '50px !important',
            width: '50px !important'
        },
        //Sizes
        '.btn-small': {
            fontSize: '14px',
            height: '34px !important',
            minHeight: 'auto',
            padding: '8px 48px',
        },
        '.btn-medium': {
            fontSize: '1rem',
            height: 'auto',
            minHeight: '50px'
        },
        '.btn-large': {
            fontSize: '1.875rem',
            height: '68px !important',
            minHeight: 'auto',
        },
        //Sizes + Circles
        '.btn-large.btn-circle': {
            height: '68px !important',
            width: '68px !important',
        },
        '.btn-small.btn-circle': {
            height: '34px !important',
            width: '34px !important'
        },
        '.btn-medium.btn-circle': {
            height: '50px !important',
            width: '50px !important'
        }
    }
}